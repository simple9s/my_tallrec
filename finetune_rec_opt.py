...
def train(
    ...
    # 采样训练数据
    if sample > -1:
        # 因为是1:19的比例，所以sample*20才是实际的样本数
        actual_sample = sample * 20
        train_data["train"] = train_data["train"].shuffle(seed=seed).select(
            range(min(actual_sample, len(train_data["train"]))))
    else:
    ...
    def compute_metrics(eval_preds):
        ...
        hit1 = compute_hit_at_k(predictions, labels, k=1)
        return {'hit@1': hit1}

    def preprocess_logits_for_metrics(logits, labels):
        """
        提取Yes/No token的logits用于计算Hit@1
        """
        # 根据模型类型获取Yes和No的token ID
        ...
        # 获取预测位置的logits（在Yes/No token之前的位置）
        ...
        # 提取Yes和No token的概率
        pred_logits = logits[labels_index[:, 0], labels_index[:, 1]][:, [no_token_id, yes_token_id]]
        pred_probs = torch.softmax(pred_logits, dim=-1)

        # 返回Yes的概率和真实标签
        return pred_probs[:, 1].cpu().numpy(), gold.cpu().numpy()
    ...
    trainer = transformers.Trainer(
        ...
        args=transformers.TrainingArguments(
            ...
            metric_for_best_model="eval_hit@1",
