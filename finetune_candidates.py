...
def train(
    ...
    train_data["train"] = (train_data["train"].shuffle(seed=seed).select(range(sample))
                           if sample > -1 else train_data["train"].shuffle(seed=seed))
    ...
    if sample > -1:
        eval_step = 10 if sample <= 128 else int(sample / 128 * 5)
    else:
        eval_step = 200
